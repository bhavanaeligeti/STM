...
<div class="mx-auto">
    <div class="">
        ...
        <div class="alert alert-success w-100 d-block"><strong>{{ $message }}</strong></div>
    @endif
    </div>
        <form method="POST" action="{{ route('upload.document.teachers') }}" class="mx-auto" enctype="multipart/form-data">
            @csrf

        <div class="form-group input-group mx-auto">
            <input name="imagetimetable" class="form-control" value="{{ old('file') }}" type="file">

        </div>
        @error('imagetimetable')
        <div class="alert alert-danger">{{ $message }}</div>
    @enderror

        <div class="form-group">
            <button type="submit" class="btn btn-primary btn-block"> Upload Dcoument  </button>
        </div> <!-- form-group// -->
    </form>
